...
この設定により、`xargs`コマンドを単体で呼び出す場合のみがバリデーションの対象になります。`find ... | xargs ...`のように先頭が`find`で始まるコマンドはfindバリデータが担当します。
...
#### 11. `builtin_configs/validate_find.yaml`
- **役割**: findコマンドバリデータのビルトイン設定
- **内容**:
  - findコマンドの参照系操作のみを許可するホワイトリスト型判定ルール
  - 安全な操作（検索オプション、表示アクション、-execによる参照系コマンド、参照系パイプ）
  - 危険な操作（-delete、-execdir、-exec rm等の破壊的コマンド、シェル構文）
  - `find ... | xargs ...`コマンドのxargs部分の安全性判定もこのバリデーターが担う
- **使用方法**: `--builtin validate_find`で指定

#### 12. `builtin_configs/validate_xargs.yaml`
- **役割**: xargsコマンドバリデータのビルトイン設定
- **内容**:
  - xargsコマンドの参照系操作のみを許可するホワイトリスト型判定ルール
  - 安全な操作（grep、cat、head、tail、wc、file、stat、ls等の参照系コマンド）
  - 危険な操作（xargs rm、xargs chmod、xargs sh -c、xargs sed -i等の破壊的コマンド）
  - xargsを単体で呼び出す場合（`xargs`で始まるコマンド）のみを対象とする
- **使用方法**: `--builtin validate_xargs`で指定
...
│   ├── validate_find.yaml       # findバリデータ設定
│   ├── validate_git_push.yaml   # Git Pushバリデータ設定
│   └── validate_xargs.yaml      # xargsバリデータ設定
├── src/
│   ├── __init__.py              # 空
